...
- (void)startPrefetchingAtIndex:(NSUInteger)index
{
    ...
    [self.manager downloadWithURL:self.prefetchURLs[index] options:self.options progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished)
    {
        ...
        if (image)
        {
            NSLog(@"Prefetched %ld out of %ld", (unsigned long)self.finishedCount, (unsigned long)self.prefetchURLs.count);
        }
        else
        {
            NSLog(@"Prefetched %ld out of %ld (Failed)", (unsigned long)self.finishedCount, (unsigned long)[self.prefetchURLs count]);
...
- (void)reportStatus
{
    ...
    NSLog(@"Finished prefetching (%u successful, %ld skipped, timeElasped %.2f)", total - self.skippedCount, (unsigned long)self.skippedCount, CFAbsoluteTimeGetCurrent() - self.startedTime);
}
